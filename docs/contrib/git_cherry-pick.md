...
- The release branch exists (example: [`release-1.18`](https://github.com/JettyJiang/open-im-server/tree/release-v3.1))
- The normal git and GitHub configured shell environment for pushing to your openim-server `origin` fork on GitHub and making a pull request against a configured remote `upstream` that tracks `https://github.com/JettyJiang/open-im-server.git`, including `GITHUB_USER`.
...
- Run the [cherry pick script](https://github.com/JettyJiang/open-im-server/tree/main/scripts/cherry-pick.sh)
  ...
  - Be aware the cherry pick script assumes you have a git remote called `upstream` that points at the openim-server github org.
    ...
    Please see our [recommended Git workflow](https://github.com/JettyJiang/open-im-server/blob/main/docs/contributors/github-workflow.md#workflow).

  - You will need to run the cherry pick script separately for each patch release you want to cherry pick to. Cherry picks should be applied to all [active](https://github.com/JettyJiang/open-im-server/releases) release branches where the fix is applicable.
